...
def belt_family_meets_minimum(family: object, minimum_family: object) -> bool:
    ...
    A missing family still satisfies Default+ so the all-tiers rule preserves
    alerts when OCR confirms the droid name but cannot read its family label.
    Higher minimums require a known family to avoid sending the wrong alert.
    """
